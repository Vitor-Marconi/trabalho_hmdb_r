...
#Way of representing chemical structures.

smiles <- structures$Smiles

#Number of atoms in the molecule.

NumAtoms <- structures$NumAt

#indicates molecular formula and atomic organization
...
#The exact Ring Count of the molecule.
...
histograma<-hist(structures$NumAtoms, breaks = 10, col="darkblue", xlim=c(10,40),main="Histograma Ring Count normal curve ");
xfit<-seq(min(structures$NumAtoms),max(structures$NumAtoms))
yfit<-dnorm(xfit,mean=mean(structures$NumAtoms),sd=sd(structures$NumAtoms))
yfit <- yfit*diff(histograma$mids[1:2])*length(structures$NumAtoms)
lines(xfit, yfit, col="red", lwd=2)

hist(RingCount, col = "purple", xlab="Ring Count", main = "Histogram of ring count", xlim = c(0,10))

hist(NumAtoms, col = "darkblue", xlab = "NumAtoms", main = "histogram of NumAtoms", xlim = c(0,150))
...
#Plot "ExactMolWt x NumAtoms".

ggplot(structures) +
  aes(x = NumRotableBonds, y = NumAtoms, colour = RingCount) +
  geom_point(shape = "circle", size = 1.5) +
  scale_color_viridis_c(option = "viridis", direction = 1) +
  labs(title = "ExactMolWt x NumAtoms") +
  theme_bw()

#Plot 'NumRotableBonds x NumAtoms'.
...
#plot NumrotableBonds x NumHAcceptors.
